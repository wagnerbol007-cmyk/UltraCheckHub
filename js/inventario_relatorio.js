...
export function gerarHistAuditoria(tipo) {
    ...
    database.ref('sinc_rfid/' + state.lojaAtual).once('value', s_sinc => {
        ...
        database.ref('arquivos_lojas/' + state.lojaAtual).once('value', s => {
            const d = s.val(); let list = [];
            if(tipo === 'padrao') { 
                Papa.parse(d.mogix, {header:true, complete: r => { 
                    list = r.data.map(i => ({ Material: normalizarCodigo(i.SKU) })); 
                    finalizarRel(list, tipo); 
                }}); 
            }
            else { 
                list = JSON.parse(d.busca || "[]").map(r => ({ Material: normalizarCodigo(r.Material || r.SKU || Object.values(r)[0]) })); 
                finalizarRel(list, tipo); 
            }
        });
    });
...
function finalizarRel(lista, tipo) { 
    ...
    lista.forEach((it) => {
        ...
        d.style = `padding: 15px; border-bottom: 1px solid #f1f5f9; font-size: 0.85em; ${val ? 'background: #f0fdf4; border-left: 8px solid #22c55e;' : ''}`;
        
        const nomeExibicao = info["Descrição material"] || moj["SKU Description"] || "Item s/ Nome";
        let tam = "--";
        for(let key in info) { if(key.toLowerCase().includes("tam")) tam = info[key]; }

        d.innerHTML = `<span style="font-size:0.7em;">EAN: ${info.EAN || '---'} | TAM: ${tam}</span><br><b>${it.Material}</b> - <span style="font-size:0.85em; color:#6200ee;">${nomeExibicao}</span>
...
export function baixarCSVInventario() {
    let csv = "\ufeffSKU;EAN;TAMANHO;NOME;ESPERADO;LIDO;ACHADO_MAO\n";
    document.getElementById('histRelatorioInv').querySelectorAll('b').forEach(node => {
        ...
        const nome = (info["Descrição material"] || moj["SKU Description"] || "Sem Nome").replace(/;/g," ");
        let tam = "";
        for(let key in info) { if(key.toLowerCase().includes("tam")) tam = info[key]; }
        
        csv += `${sku};${info.EAN || ''};${tam};${nome};${moj.Expected || 0};${moj.Counted || 0};${state.respostasInventarioNuvem[sku] || ''}\n`;
    });
    const blob = new Blob([csv], {type: 'text/csv;charset=utf-8;'});
    const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `Auditoria_Inv_Lj${state.lojaAtual}.csv`; link.click();
}
