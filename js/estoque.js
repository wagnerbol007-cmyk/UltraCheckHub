...
export function ouvirEstoque() {
    database.ref(`reposicao_ativa/${state.lojaAtual}`).on('value', s => {
        ...
        Object.entries(s.val()).forEach(([id, item]) => {
            ...
            const action = document.createElement('div'); action.className = 'swipe-action'; action.innerHTML = "NÃO<br>ACHEI";
            
            action.onclick = () => moverParaAnalise(id, item);
            ...
            card.addEventListener('touchmove', e => {
                ...
                if(Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 40) {
                    ...
                    if(dx > 0 && card.style.transform.includes('-')) card.style.transform = `translateX(${Math.min(-100 + dx, 0)}px)`;
                }
            }, {passive: true});
            card.addEventListener('touchend', e => { 
                let cur = parseInt(card.style.transform.replace(/[^-0-9]/g, '') || 0); 
                if(cur < -50) card.style.transform = `translateX(-100px)`; 
            ...
            wrap.appendChild(action); wrap.appendChild(card); container.appendChild(wrap);
        });
